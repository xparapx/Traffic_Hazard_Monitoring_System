...
### 프로젝트 지도 v2 — 6뷰 확장 (2026-09-22)

3뷰(설계·연결·DB) 지도를 스킬 최신 규격으로 확장: **시나리오 3편**(등교 Fast Loop · 위험 이벤트 Slow Loop · 주간 리포트 승인 흐름) + **데이터 여정 5줄**(무저장 프레임 → 지표 → 이벤트/C1 → 분석 → 라벨) + **운영·배포 탭**(orin 서비스 3종·pull형 CD·doctor 점검) + **맞춤 용어**(K1~K3·C1·C2·GATE·FAKE_HW, "VLM은 본체가 아니다" 노트). 부제 '지속 갱신' 명시.
